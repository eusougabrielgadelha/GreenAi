...
async def morning_scan_and_publish():
    ...
    # --- ALTERAÇÃO: Define a data de análise como AMANHÃ ---
    analysis_date_local = (datetime.now(ZONE).date() + timedelta(days=1))
    logger.info("📅 Dia analisado (timezone %s): %s (AMANHÃ)", ZONE, analysis_date_local.isoformat())
    # -------------------------------------------------------
    ...
    # Resumo só com os jogos de amanhã (já filtrados)
